...
export class CardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.acctSvc.getBankCards().subscribe({
      next: r => { this.cards = r.data ?? []; this.loading = false; },
  ...
  openAdd(): void {
    this.form = { cardType: 'Visa', cardCategory: 'Debit', expiryMonth: 1, expiryYear: this.years[0] };
    this.modalError = '';
    this.modal = true;
  }
  ...
  setDefault(cardId: number): void {
    this.acctSvc.setDefaultCard(cardId).subscribe({
      next: () => {
        this.cards = this.cards.map(c => ({ ...c, isDefault: c.cardId === cardId }));
      }
    });
  }
